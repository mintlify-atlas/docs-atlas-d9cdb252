...
interface Project {
  ...
  fullDescription: string; // Full paragraph for the detail / expanded view
  links: {
...
**Specter Analytics (`prj-001`)** — A privacy-first analytics dashboard. `dataSource: "LIVE"`. Has a full case study linked at `/case-studies/specter`.

**Nexus Sync (`prj-002`)** — A real-time state synchronization engine built on Rust and WebSockets. `dataSource: "SAMPLE"`. Active development in progress.

**Legacy API Wrapper (`prj-003`)** — A modern GraphQL layer over a 20-year-old SOAP API. `dataSource: "MOCKED"`. Status: HAUNTED — maintained but approached carefully.

**Terminal Portfolio (`prj-004`)** — A previous portfolio site rendered as a fake CLI terminal. `dataSource: "LIVE"`. Retired and preserved as a historical artifact.
